...
export async function getFacesAvailable() {
  return apiFetch<{ available: boolean }>("/people/available");
}

export async function getPersonClips(clusterId: string) {
  return apiFetch<Clip[]>(`/people/${clusterId}/clips`);
}

export async function namePerson(clusterId: string, name: string) {
  return apiFetch<Person>(`/people/${clusterId}/name`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
// --- Multipart video ingest (browser -> B2 direct) --------------------------
 ...
 * and kick off the multimodal index pipeline. The API never touches the bytes.
 */
