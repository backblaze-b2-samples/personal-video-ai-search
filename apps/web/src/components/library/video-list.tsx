...
import { VideoPlayerDialog } from "@/components/library/video-player-dialog";
...
export function VideoList({ videos }: { videos: Video[] }) {
  ...
  // Which video is open in the player dialog. The source object exists in B2
  // once the multipart upload completes, so playback is available for every
  // status except `uploading` (parts still landing).
      ...
      <Table className="table-fixed">
        <TableHeader>
          <TableRow className="bg-muted/40 hover:bg-muted/40">
            <TableHead className="w-[23%] text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              Video
            </TableHead>
            ...
            <TableHead className="w-[9%] text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              Scenes
            </TableHead>
            <TableHead className="w-[9%] text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              People
            ...
            <TableRow key={video.video_id} className="table-row-hover">
              ...
              <TableCell className="text-right whitespace-nowrap">
                <Button
                  ...
                  size="icon"
                  className="h-7 w-7"
                  title={video.status === "uploading" ? "Still uploading" : "Watch"}
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        derived data (audio, transcript, thumbnails, scene tags,
                        embeddings) from B2. This cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
