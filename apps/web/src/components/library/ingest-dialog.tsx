  ...
  const onDrop = useCallback(
    (accepted: File[]) => {
      ...
      ingest.mutate(
        ...
        {
          onSuccess: (video) => {
            toast.success(`"${video.title}" uploaded — indexing started.`);
            setOpen(false);
            setProgress(0);
          },
          onError: (err) => {
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            multi-GB files are fine. Transcription, scene tagging, and face
            clustering start automatically once the upload completes.
          </DialogDescription>
        </DialogHeader>
        {ingest.isPending ? (
