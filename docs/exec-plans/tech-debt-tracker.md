...
| Date/event filter in Search is stubbed | Search supports person filter but not date/event yet | Add `created_at` range + event-name filter to `SearchRequest` and the UI | Low | Open |
