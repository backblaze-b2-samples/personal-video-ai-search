...
# External SDKs that must stay behind repo/ adapters. The multimodal pipeline
# adds AI providers and the local face stack to the original boto3 rule.
# numpy/scikit math used by the service layer for cosine scoring is NOT an
# external service client, so it is deliberately not listed here.
EXTERNAL_SDKS = (
    "boto3",
    "botocore",
    "openai",
...
def test_external_sdks_only_in_repo():
    """Verify external SDKs (boto3 + AI providers + face stack) are only
    imported in app/repo/."""
    violations = []
    for layer in LAYER_ORDER:
        if layer == "repo":
            continue
