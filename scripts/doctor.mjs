...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-personal-video-ai-search)",
    );
  }
}
...
function checkFfmpeg() {
  // Warn (not fail): ffmpeg/ffprobe are only needed for the video index
  // pipeline (audio extraction, keyframe sampling, probing). The generic
  // B2 surfaces (upload, files, dashboard) run fine without them.
  if (!tryExec("ffmpeg -version") || !tryExec("ffprobe -version")) {
    warn(
      "ffmpeg/ffprobe not found — the video index pipeline can't extract audio or sample keyframes without them",
      "Install ffmpeg: `brew install ffmpeg` (macOS) or `apt-get install ffmpeg` (Debian/Ubuntu).",
    );
  }
}
